...
def create_mipmap_from_tuple(mipmap_tuple, levels=[1,2,3], convertTo8bit=True, force_redo=True):
    (filepath,downdir)=mipmap_tuple
    print mipmap_tuple
    return create_mipmaps(filepath, outputDirectory=downdir, mipmaplevels=levels, convertTo8bit=convertTo8bit, force_redo=force_redo)

def make_tilespecs_and_cmds(render, inputStack, output_dir, zvalues, levels, imgformat, convert_to_8bit, force_redo, pool_size):
    mipmap_args = []
    ...
    mypartial = partial(create_mipmap_from_tuple, levels=range(1,levels+1), convertTo8bit=convert_to_8bit, force_redo=force_redo)
...
class GenerateMipMapsParameters(RenderParameters):
    ...
    output_dir = mm.fields.Str(required=True,
        metadata={'description':'directory to which the mipmaps will be stored'})
    method = mm.fields.Str(required=True, default="block_reduce",
    ...
    imgformat = mm.fields.Str(required=False, defautl='tiff',
        metadata={'description':'image format for mipmaps (default tiff)'})
    levels = mm.fields.Int(required=False,default=6,
        metadata={'description':'number of levels of mipmaps, default is 6'})
    force_redo = mm.fields.Boolean(required=False, default=True,
        metadata={'description':'force re-generation of existing mipmaps'})
    ...
    z = mm.fields.Int(required=False,
        metadata={'description':'z-index in the stack'})
...
class GenerateMipMaps(RenderModule):
    def __init__(self, schema_type=None, *args, **kwargs):
        ...
        super(GenerateMipMaps,self).__init__(schema_type=schema_type, *args, **kwargs)
    ...
    def run(self):
        ...
        if self.args['method'] == 'PIL' or self.args['method'] == 'block_reduce':
