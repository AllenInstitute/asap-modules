...
class PointMatchOpenCVParameters(RenderParameters):
    ndiv = Int(
        ...
        default=8,
        missing=8,
        description="one tile per tile pair subdivided into ndiv x ndiv for easier homography finding")
    matchMax = Int(
        required=False,
        default=1000,
        missing=1000,
        description="per tile pair limit, randomly chosen after SIFT and RANSAC")
    ...
    RANSAC_outlier = Float(
        ...
        default=5.0,
        missing=5.0,
        description="passed to cv2.findHomography(src, dst, cv2.RANSAC, outlier)")
    FLANN_ntree = Int(
        required=False,
        default=5,
        missing=5,
