...
def swap_section(render, source_stack, target_stack, z, pool_size=5):
    ...
    source_ts = renderapi.resolvedtiles.get_resolved_tiles_from_z(
        source_stack, z, render=render)
    target_ts = renderapi.resolvedtiles.get_resolved_tiles_from_z(
        target_stack, z, render=render)
    ...
    if set(source_tileids) & set(target_tileids):
        print("{} and {} stack have same tileIds for Z = {}".format(
            source_stack, target_stack, z))
        return False
    ...
    try:
        ...
        print("Cannot import tilespecs to {} from {}".format(
    ...
    try:
        for ts in source_ts.tilespecs:
            renderapi.stack.delete_tile(
                source_stack, ts.tileId, session=session, render=render)
    except Exception as e:
        print("Cannot delete old tile ids from {}, error: {}".format(
            source_stack, e))

    try:
        for ts in target_ts.tilespecs:
            renderapi.stack.delete_tile(
                target_stack, ts.tileId, session=session, render=render)
    except Exception as e:
        print("Cannot delete old tile ids from {}, error: {}".format(
            target_stack, e))
...
class SwapZsModule(RenderModule):
    ...
    def run(self):
        ...
        target_stacks = []
        zvalues = []
        for source_stack, target_stack, zVals in zip(
                self.args['source_stack'],
                self.args['target_stack'],
                self.args['zValues']):
            if ((source_stack not in list_of_stacks) or (  # pragma: no cover
                    target_stack not in list_of_stacks)):  # pragma: no cover
                continue
            ...
            if self.args['complete_source_stack']:
                renderapi.stack.set_stack_state(
                    source_stack, state='COMPLETE', render=self.render)

            if self.args['complete_target_stack']:
                renderapi.stack.set_stack_state(
                    target_stack, state='COMPLETE', render=self.render)
