...
def draw_matches1(render, stack, tile1, tile2, ptmatches, scale, outdir, color=None):
...
def draw_matches(im1, im2, ptmatches, scale, outdir, color=None):
    print(im1)
    print(im2)
    img1 = cv2.imread(im1)
    img2 = cv2.imread(im2)
    ...
    # if not resize the smallest to the largest
    if img1.shape > img2.shape:
        img2 = cv2.resize(img2, img1.shape)
...
def get_tile_pair_matched_image(render, stack, tileId1, tileId2, pGroupId, qGroupId, outdir, matchCollectionOwner, matchCollection, renderScale=0.1):
    ...
    #im1 = '%s_%f.jpg'%(tileId1, renderScale)
    #im2 = '%s_%f.jpg'%(tileId2, renderScale)
    im1 = '%s.jpg'%tileId1
    im2 = '%s.jpg'%tileId2
    ...
    #match_img_filename = draw_matches(im1, im2, ptmatches, renderScale, outdir)
    match_img_filename = draw_matches1(render, stack, tileId1, tileId2, ptmatches, renderScale, outdir)
...
def compute_point_matches(render, stack, tileID, output_dir, url_options, option_keys, options):
    ...
    #outdir = os.path.join(output_dir, '%s'%str(ops['renderScale']))
    ...
    def run(self):
        ...
        for tileids in tileIds:
            return_struct.append(compute_point_matches_for_tilepair(self.render, 
                                                                    ...
                                                                    tileids))
        '''
        # mypartial with iterating over options rather than tilepairs
        # this introduces a race condition with saving tilepair images
        ...
        print(len(return_struct))
        m = render_from_template(os.path.dirname(__file__), 'optimization1.html', return_struct=return_struct, zipped=zipped)
