...
from rendermodules.pointmatch.swap_point_match_collections import SwapPointMatchesModule
...
def test_swap_pt_matches_module(render, pt_matches1, pt_matches2):
    ...
    sgroupIds = renderapi.pointmatch.get_match_groupIds(pt_matches1, 
                                                       owner=example['match_owner'],
                                                       render=render)
    tgroupIds = renderapi.pointmatch.get_match_groupIds(pt_matches2,
                                                        owner=example['match_owner'],
                                                        render=render)
    ...
    target_pIds = []
    for sg in sgroupIds:
        matches = renderapi.pointmatch.get_matches_with_group(pt_matches1,
                                                              sg,
                                                              owner=example['match_owner'],
                                                              render=render)
        for m in matches:
    ...
    for tg in tgroupIds:
        matches = renderapi.pointmatch.get_matches_with_group(pt_matches2,
                                                              tg,
                                                              owner=example['match_owner'],
                                                              render=render)
        for m in matches:
    ...
    mod = SwapPointMatchesModule(input_data=example, args=['--output_json', 'out.json'])
    mod.run()
    ...
    matches = renderapi.pointmatch.get_matches_with_group(pt_matches1, '1015.0', owner=example['match_owner'], render=render)
    new_source_pIds = [m['pId'] for m in matches]

    matches = renderapi.pointmatch.get_matches_with_group(pt_matches2, '1015.0', owner=example['match_owner'], render=render)
