...
def test_rough_registration(render, reference_stack, moving_stack, point_match_collection):
    ...
    rtspecs = renderapi.tilespec.get_tile_specs_from_z(example['reference_stack'], example['reference_z'], render=render)
    
    match = renderapi.pointmatch.get_matches_from_tile_to_tile(example['match_collection'],
    ...
    mtspecs = renderapi.tilespec.get_tile_specs_from_z(example['output_stack'], example['moving_z'], render=render)
    mtform = mtspecs[0].tforms[0]
    assert(abs(mtform.shear-0.0) < 10.0 * np.finfo(float).eps)
