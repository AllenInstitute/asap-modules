...
class RegisterSectionSchema(RenderParameters):
    reference_stack = Str(
        required=True,
        descritption=(
            "Reference stack to which section needs to be registered to"))
    moving_stack = Str(
        required=True,
        description=(
            "Moving stack with section that needs to be "
            "registered to ref stack"))
    output_stack = Str(
        required=True,
        description=(
            "Output stack with moving section registered to ref section"))
    match_collection = Str(
        required=True,
        description=(
            "Match collection storing the pt matches between "
            "the ref and moving section tiles"))
    match_owner = Str(
        required=False,
        default=None,
        missing=None,
    ...
    registration_model = Str(
        ...
        description=(
            ...
            "(Affine or Rigid) Default - Affine"))
    reference_z = Int(
        required=True,
    ...
    overwrite_output = Boolean(
        ...
        default=True,
        missing=True,
        description=("Do you want to overwrite output stack if "
                     "section already exists in it (Default - True)"))
    consolidate_transforms = Boolean(
        required=False,
        default=True,
        missing=True,
        description=(
            "Consolidate transforms in tilespecs if "
...
class RegisterSectionOutputSchema(DefaultSchema):
    out_stack = Str(
        required=True,
        description=(
            "output stack to which the registered section is written to"))
    registered_z = Int(
        required=True,
