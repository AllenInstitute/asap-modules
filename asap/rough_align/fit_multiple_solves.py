...
example = {
    ...
    "pool_size": 20, #customizable. 20 is probably overkill. Will be used in any inherent concurrency in the module.
    "close_stack": False, #Must remain False to keep the stack open so new ROI can be added.
...
def create_resolved_tiles(render,pm_render,input_stack: str,pm_collection: str,solve_range: tuple, pool_size: int):
    #input_stack and pm_collection are names of the (a) montage-scaped & Z-mapped stack, and
    #(b) pointmatch collection from render, respectively; solve_range is a tuple of the minimum and maximum of the new_z values.
...
def nomatchz_counter(combined_rts,tIds_nomatch):
    nomatchz_count = collections.Counter([combined_rts[tId].z for tId in tIds_nomatch]) #should be nothing in it if everything works well
    if len(nomatchz_count) > 0:
        raise TileException(f'Unmatched TileIds Exist: {list(map(int, sorted({tId: combined_rts[tId].z for tId in tIds_nomatch})))}')
    return None
...
def create_input_defaults(render,pm_render,input_stack: str,pm_collection: str,solve_range: tuple, pool_size:int,multiz=False):
    combine_resolvedtiles = renderapi.resolvedtiles.combine_resolvedtiles
    res_tiles = create_resolved_tiles(render,pm_render,input_stack=input_stack,pm_collection=pm_collection,solve_range=solve_range,pool_size=pool_size)
...
def rigid_solve(combined_rts,new_matches):
    ...
    rigid_create_CSR_A_input = (combined_rts, rig_matches, rigid_transform_name,
...
class FitMultipleSolves(argschema.ArgSchemaParser):
    ...
    def apply_transform(self,new_matches,rts,app):
        ...
        app_sol = bigfeta.solve.solve(app_fr["A"], app_fr["weights"], app_fr["reg"], app_fr["x"], app_fr["rhs"])
    ...
    def run(self):
        ...
        minZ = (allZ[0] if self.args["minZ"] is None else max(self.args["minZ"],allZ[0]))
        maxZ = (allZ[-1] if self.args["maxZ"] is None else min(self.args["maxZ"],allZ[-1]))

        sol_range = (minZ,maxZ)

        in_default = create_input_defaults(render=r_in,pm_render=r_pm,input_stack=self.args['input_stack']['name'],
                                        pm_collection=self.args['pointmatch_collection']['name'],
                                        solve_range=sol_range,pool_size=self.args['pool_size'],multiz=False)

        '''---------------------Rigid solve-------------------------'''
        rigid_result_rts=self.apply_transform(new_matches=in_default['new_matches'],rts=in_default['combined_tilespecs'],app='rigid')
        self.save_transform(render=r_rot,rts=rigid_result_rts,app='rigid')
        if do_translate:
            '''---------------Translation solve -------'''
            trans_result_rts=self.apply_transform(new_matches=in_default['new_matches'],rts=rigid_result_rts,app='translate')
            self.save_transform(render=r_trans,rts=trans_result_rts,app='translate')
            '''---------------Affine solve ------------'''
            aff_result_rts = self.apply_transform(new_matches=in_default['new_matches'],rts=trans_result_rts,app='affine')
        else:
            '''---------------Affine solve ------------'''
            aff_result_rts = self.apply_transform(new_matches=in_default['new_matches'],rts=rigid_result_rts,app='affine')
        self.save_transform(render=r_aff,rts=aff_result_rts,app='affine')
            
        '''---------------Thin_plate_spline solve--------------------'''
        tps_result_rts = self.apply_transform(new_matches=in_default['new_matches'],rts=aff_result_rts,app='tps')
        self.save_transform(render=r_tps,rts=tps_result_rts,app='tps')
        

        allZ_out = [int(z) for z in renderapi.stack.get_z_values_for_stack(self.args["rigid_output_stack"]['name'], render=renderapi.connect(**self.args["rigid_output_stack"]))]
